...
def test_simple_rule_chanining_with_insert():
    rule_1 = Rule(id='r1',
                when=Condition(for_type=P1, matches_exp=lambda ctx, this: this.val > 0 and assign(ctx, parent=this)),
                then=lambda ctx: insert(ctx, Ch1(ctx.parent, 20)))
    rule_2 = Rule(id='r2',
                when=Condition(for_type=Ch1, matches_exp=lambda ctx, this: this.val > 0 and assign(ctx, child=this)),
...
def test_rule_chanining_with_insert_and_matching():
    ...
    rule_2 = Rule(id='r2', 
                    when=[
                        Condition(for_type=P1, matches_exp=lambda ctx, this: this.val > 0 and assign(ctx,parent=this)),
...
def test_simple_rule_chanining_with_update():
    def zero_out(ctx):
        ...
        update(ctx, ctx.c1)
    rule_1 = Rule(id='r1',
                when=Condition(for_type=C1, matches_exp=lambda ctx, this: this.val > 0 and assign(ctx, c1=this)),
                then=zero_out)
    rule_2 = Rule(id='r2',
...
def test_rule_chanining_with_delete_and_matching():
    ...
    rule_2 = Rule(id='r2', order=1,
                when=[
                    Condition(for_type=P1, matches_exp=lambda ctx, this: this.val > 0 and assign(ctx,parent=this)),
                    Condition(for_type=Ch1, matches_exp=lambda ctx, this: this.val > 0 and assign(ctx,child=this) and this.parent == ctx.parent)],
                then=lambda ctx: insert(ctx, R1(ctx.parent, ctx.child)))
    parent = P1(20)
